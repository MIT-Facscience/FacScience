...
import { Mail, Phone, ChevronDown, ChevronUp, Building2, TreePine } from "lucide-react"
...
const PersonCard: React.FC<{ person: PersonData; className?: string }> = ({ person, className = "" }) => (
  <Card className={`hover:shadow-lg transition-shadow duration-200 ${className}`} style={{ borderRadius: 0 }}>
    <CardHeader className="pb-3" style={{ borderRadius: 0 }}>
      <CardTitle className="text-lg font-semibold text-balance" style={{ borderRadius: 0 }}>
        {person.prenom} {person.nom}
      </CardTitle>
      <Badge variant="secondary" className="w-fit text-xs" style={{ borderRadius: 0 }}>
    ...
    <CardContent className="space-y-2" style={{ borderRadius: 0 }}>
      ...
      {person.tel && (
        <div className="flex items-center gap-2 text-sm text-muted-foreground">
          ...
          <a href={`tel:${person.tel}`} className="hover:text-primary transition-colors">
            {person.tel}
          </a>
        </div>
      )}
        ...
        <div className="space-y-2">
          ...
          {member.email && (
            <div className="flex items-center gap-2 text-xs text-muted-foreground group-hover:text-white transition-colors duration-300">
              ...
              <a href={`mailto:${member.email}`} className="hover:underline transition-all duration-200 truncate">
                {member.email}
              </a>
            </div>
          )}
          {member.telephone && (
            <div className="flex items-center gap-2 text-xs text-muted-foreground group-hover:text-white transition-colors duration-300">
              ...
              <a href={`tel:${member.telephone}`} className="hover:underline transition-all duration-200">
                {member.telephone}
              </a>
            </div>
          )}
...
const HierarchyNode: React.FC<{
  ...
  person?: PersonData
  isService?: boolean
}> = ({ title, level, children, person, isService = false }) => {
  const getNodeStyle = () => {
    switch (level) {
      case 0:
        return {
      ...
      >
        ...
        {person && person.email && (
          <div className="flex items-center gap-1 text-xs opacity-80 mt-1">
            <Mail className="h-3 w-3" />
            <a href={`mailto:${person.email}`} className="hover:opacity-100 transition-opacity">
              {person.email}
            </a>
          </div>
        )}
...
export default function Organigramme() {
  ...
  const doyen = { nom: "DOYEN", prenom: "", responsabilite: "Doyen", email: "", tel: "", id: 0 }
