  ...
  return (
    <div className="min-h-16 bg-gradient-to-br from-slate-50 via-purple-50/30 to-amber-50/20">
      ...
      <header className="fixed top-0 left-0 right-0 z-50 w-full">
          ...
          <div className="container mx-auto px-6">
            <div className="flex h-18 items-center justify-between">
              ...
              <nav className="hidden lg:flex items-center space-x-1">
                ...
                <a href="/actualites" className="px-5 py-3 text-slate-700 hover:text-purple-700 transition-all duration-300 font-medium text-sm tracking-wide hover:bg-purple-50/50 rounded-lg relative group">
                  Actualités
                ...
                <a href="/resultats" className="px-5 py-3 text-slate-700 hover:text-purple-700 transition-all duration-300 font-medium text-sm tracking-wide hover:bg-purple-50/50 rounded-lg relative group">
                  Résultats
                  <div className="absolute bottom-1 left-1/2 transform -translate-x-1/2 w-0 h-px bg-gradient-to-r from-purple-400 to-amber-400 transition-all duration-300 group-hover:w-8"></div>
                </a>
        ...
        {isOpen && (
          <div className="lg:hidden fixed inset-0 z-40 bg-slate-900/20 backdrop-blur-sm" onClick={() => setIsOpen(false)}>
            <div 
            ...
            >
              <div className="p-8 space-y-8">
                ...
                {[
                  { title: "Présentation", items: presentationItems, accent: "border-purple-300" },
                  { title: "Formation", items: formationItems, accent: "border-amber-300" },
                  { title: "Recherche", items: rechercheItems, accent: "border-purple-400" }
                ].map((section, sectionIndex) => (
                  <div key={section.title} className="space-y-3">
                    <h3 className={`font-semibold text-slate-800 text-sm tracking-wide uppercase border-b ${section.accent} pb-2`}>
                      {section.title}
                    </h3>
                ...
                <div className="space-y-3 pt-6 border-t border-purple-200/50">
                  {[
                    { name: "Actualités", gradient: "from-purple-50 to-purple-100", hover: "hover:from-purple-100 hover:to-purple-150", border: "border-purple-200" },
                    { name: "Résultats", gradient: "from-amber-50 to-amber-100", hover: "hover:from-amber-100 hover:to-amber-150", border: "border-amber-200" },
                    { name: "Contact", gradient: "from-purple-600 to-purple-700", hover: "hover:from-purple-500 hover:to-purple-600", border: "border-purple-600", text: "text-white" }
                  ].map((link) => (
                    <a
                      key={link.name}
                      href={`/${link.name.toLowerCase()}`}
                      className={`block p-4 bg-gradient-to-r ${link.gradient} ${link.hover} rounded-xl transition-all duration-200 font-medium ${link.text || 'text-slate-800'} border ${link.border}`}
                      onClick={() => setIsOpen(false)}
                    >
                      {link.name}
                    </a>
